...
- **Execution dispatch** on the response `type`: `evm` (approve to `approveTo` unless native/`skipApproval`, then send calldata), `btc` (transfer to generated deposit address; uses `/v2/swap`, rate-limited 1 rps), `ton` (single-message routes only — the TON wallet reads a fresh seqno per send without waiting for inclusion, so sequential sends race and multi-message routes are refused; the message's base64 BoC payload goes as `body`), `tron` (approve in Tron hex form `41…`, then smart contract call with `rawParameter`; receipts are raw tronweb `getTransactionInfo` objects — failure is `result: 'FAILED'`/`receipt.result`, not `status`), `solana` (pass `tx.instructions` — a base64 serialized transaction — to `sendTransaction`). For ton/tron/solana the module first probes the bound account (`_canExecuteTon` builds a throwaway message offline and checks the probe BoC isn't comment-wrapped; `_canExecuteTron` checks the `_isSmartContractCall` class marker + `approve` for token inputs; `_canExecuteSolana` checks `_signSerializedTransaction`) and throws `UnsupportedRouteError` when unsupported — older wdk-wallet versions: TON wallet encodes string bodies as text comments (fund-loss), Tron wallet (< beta.9) has no contract calls and no approve (PR tetherto/wdk-wallet-tron#52), Solana wallet doesn't accept serialized transactions (PR tetherto/wdk-wallet-solana#78), TON raw BoC pending PR tetherto/wdk-wallet-ton#78.
